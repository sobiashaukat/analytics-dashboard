...
const Badge = ({percentage}: {percentage:number}) => {
    ...
    return(
        <span className={"inline-flex gap-1 items-center rounded-md px-2 py-1  text-xs  font-medium ring-1 ring-inset ${  isPositive ?positiveClassName: isNeutral ? neutralClassname : nagativeClassname: isNegative}"}>
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <Card className="flex flex-col sm:grid grid-cols-4 gap-6">
        <h2 className="w-full text-dark-tremor-content-strong text-center
        sm:left-left font-semibold text-xl">
            This week top visitors:
        </h2>
        <div className="col-span-3 flex items-center  justify-between
        flex-wrap gap-8">
            {topCountries?.map(([countryCode,number]) => {
                return <div key={number} className="flex items-center gap-3
                text-dark-tremor-content-strong">
                    ...
                    sm:text-dark-tremor-content-strong">{number}</p>
